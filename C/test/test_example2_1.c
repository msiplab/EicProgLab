...
void test_example2_1_norm_should_be_1_for_unit_vector(void) {
    const char* input = "3\n1\n0\n0\ny\n0\n1\n0\ny\n0\n0\n1\nn\n";  // 入力: 次元=3, ノルム1, 継続2回
    ...
    // 出力に "1.000000" を含むことを確認（ノルム）
    TEST_ASSERT_NOT_NULL(strstr(buffer, "1.000000"));
    // 呼び出し回数など他の出力も確認可能（任意）
    TEST_ASSERT_NOT_NULL(strstr(buffer, "getNorm の呼出し回数: 1"));
    TEST_ASSERT_NOT_NULL(strstr(buffer, "getNorm の呼出し回数: 2"));
    TEST_ASSERT_NOT_NULL(strstr(buffer, "getNorm の呼出し回数: 3"));        
}

int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_example2_1_norm_should_be_5_for_3_4_vector);
    RUN_TEST(test_example2_1_norm_should_be_1_for_unit_vector);
    return UNITY_END();
}
